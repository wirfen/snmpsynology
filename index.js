...
var session = new snmp.Session({ host: '192.168.1.2', port: 161, community: 'wirfen' });
var entrada=0;
var salida=0;
...
var oids = [[1,3,6,1,4,1,6574,1,1,0],		// 00 systemStatus
	...
	[1,3,6,1,2,1,31,1,1,1,6,7],		// 10 ifHCInOctets (bits)
	[1,3,6,1,2,1,31,1,1,1,10,7],		// 11 ifHCOutOctets (bits)
...
setInterval(function() {
	session.getAll({ oids: oids }, function (error, varbinds) {
		con.query("INSERT INTO medidas (systemStatus, temperature, diskStatus1, diskStatus2, diskTemperature1, diskTemperature2, raidFreeSize, ssCpuTotal, memAvailFree, ifHCInOctets, ifHCOutOctets, spaceIOLA, users, upsInfoStatus, upsInfoLoadValue, upsBatteryChargeValue) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [varbinds[0].value, varbinds[1].value, varbinds[2].value, varbinds[3].value, varbinds[4].value, varbinds[5].value, varbinds[6].value, varbinds[7].value + varbinds[8].value, varbinds[9].value, varbinds[10].value*8 - entrada, varbinds[11].value*8 - salida, varbinds[12].value, varbinds[13].value + varbinds[14].value + varbinds[15].value + varbinds[16].value + varbinds[17].value + varbinds[18].value + varbinds[19].value + varbinds[20].value + varbinds[21].value, varbinds[22].value, parseFloat(varbinds[23].value), parseFloat(varbinds[24].value)], function(err, result) {
			if (err) throw err;
			entrada = varbinds[10].value*8;
			salida = varbinds[11].value*8;
		});
	});
}, 5000)
